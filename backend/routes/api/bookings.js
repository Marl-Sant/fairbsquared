...
router.get(
    ...
    async (req, res) => {
        ...
        if(!findBooking){
            ...
            res.status(401)
            res.json({message: "User must be the owner of the booking to see it"})
        }else{
...
router.delete(
    ...
    async (req, res) => {
        ...
        if(!bookingToDelete){
            ...
            res.status(401)
            res.json({message: "User must be the owner of the booking to delete it"})
        }else{
            bookingToDelete.destroy()
            res.json({message: "Booking has been deleted successfully"})
        }
    }
)
...
router.put(
    ...
    async (req, res) => {
        ...
        if(!bookingToEdit){
            ...
            res.status(401)
            res.json({message: "User must be the owner of the booking to make edits"})
        }else{
